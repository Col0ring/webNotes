...
这里的使用方式都是使用create-react-app创建项目时的
...
- 在根目录创建`config-overrides.js`文件，用于覆盖配置
...
因为该框架使用less书写，所以改变主题需要确保加载了`less`和`less-loader`
...
module.exports = override(
  ...
  addLessLoader({
    ...
    /* 
    在modifyVars里面 可以修改对应的主题色了，修改基本的变量名，在使用后就会呈现不同主题色，比如我下面设置的主体的颜色为白色，主体颜色的模块被点击呈现黑色
    举个例子：设置<Button type="primary">Test</Button>,这是的按钮应该默认是白色，点击时是黑色
    */
    modifyVars: { '@brand-primary': '#fff', '@brand-primary-tap': '#000' }
  })
)
```
